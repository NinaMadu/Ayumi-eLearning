...
function Dashboard() {
    ...
    const fetchUserCount = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/users/count`
        );
  ...
  useEffect(() => {
    const fetchCourseCount = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/course/count`
        );
        const data = await response.json();
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen bg-gray-50 p-8 shadow-lg rounded-lg">
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 gap-4 mb-8">
          <div className="bg-gradient-to-r bg-red-200 text-black p-4 rounded-lg shadow-md h-40">
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <div className="text-xl sm:text-xl font-bold">{userCount}</div>
            </div>
          </div>
          <div className="bg-gradient-to-r bg-blue-200 text-black p-4 rounded-lg shadow-md h-40">
            <div className="flex flex-col items-center justify-center h-full">
              <FaBook className="mb-2 text-xl sm:text-3xl" />
              <div className="text-xl sm:text-xl font-semibold">
                Total Courses
              </div>
              <div className="text-xl sm:text-xl font-bold">{courseCount}</div>
            </div>
          </div>
          <div className="bg-gradient-to-r bg-slate-200 text-black p-4 rounded-lg shadow-md h-40">
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <div className="text-xl sm:text-xl font-bold">0</div>
            </div>
          </div>
        </div>
