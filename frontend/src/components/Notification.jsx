...
const Notification = ({ type, message, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-20 right-4 z-50 w-80 p-4 rounded-lg shadow-lg text-white ${bgColor} flex items-center justify-between transition-all duration-1000 ${fadeOut ? 'opacity-0 transform scale-95' : 'opacity-75'}`}
    >
      <p>{message}</p>
      <button
