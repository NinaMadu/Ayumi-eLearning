...
export const getSubmissionsByCourseAndAssignmentId = async (req, res) => {
    ...
    try {
        ...
        if (!submissions.length) {
            return res.status(404).json({ message: "No submissions found for the given course and assignment." });
        }
...
export const getSubmissionOfUserByCourseAndAssignmentId = async (req, res) => {
    ...
    try {
        const submission = await Submission.findOne({ courseId, assignmentId, userId })
            .populate('assignmentId', 'title')
            .populate('userId', 'firstName')
            .where({ courseId, assignmentId, userId });

        if (!submission) { 
            return res.status(404).json({ message: "No submission found for the given course, assignment, and user." });
        }
